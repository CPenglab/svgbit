...
class STDataset(object):
    ...
    count_df : np.ndarray, pd.DataFrame, str or Path
       ...
       is given, svgene will try to read file with given path with pandas.
    ...
    def acquire_density(self, cores: int = density.cpu_count()) -> None:
        ...
        ==========
        cores: int
            Number of threads to run svgene. Use all available cpus by default.
    ...
    def weight(self) -> libpysal_W:
        """Weight used by svgene. Use KNN if not specified."""
        return self._weight
